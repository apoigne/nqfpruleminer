...
class HistoryTest3 extends FunSuite {
  ...
  implicit private val ctx: Context = new Context("src/test/resources/historytest3.conf", 0, 0)
  ...
  test("History test") {
    {
      ...
      assert(
        result.head ==
          List(GroupBy(Some(Nominal("4711", 0)), Aggregated(AggregationOp.Min, 2, Some(1.0), 1, 3), 3, -1))
      )
    ...
    }
    {
      ...
      assert(
        result.head.toSet ==
          Set(
            GroupBy(Some(Nominal("4711", 0)), Aggregated(AggregationOp.Min, 2, Some(1.0), 1, 3), 3, -1),
            GroupBy(Some(Nominal("4721", 0)), Aggregated(AggregationOp.Min, 2, Some(1.0), 1, 3), 3, -1)
          )
      )
      assert(result(1).toSet ==
        Set(
      ...
      assert(
        result.head.toSet ==
          Set(
            GroupBy(Some(Nominal("4711", 0)), Aggregated(AggregationOp.Min, 2, Some(1.0), 1, 3), 3, -1),
            GroupBy(Some(Nominal("4721", 0)), Aggregated(AggregationOp.Min, 2, Some(1.0), 1, 3), 3, -1),
            GroupBy(Some(Nominal("4722", 0)), Aggregated(AggregationOp.Min, 2, Some(0.0), 1, 3), 3, -1)
          )
      )
      assert(result(1).toSet ==
        Set(
    ...
    }
    {
      ...
      assert(result(1).toSet ==
        Set(
          GroupBy(Some(Nominal("4711", 0)), Aggregated(AggregationOp.Sum, 2, Some(5.0), 1, 4), 4, -1)
        )
      )
    }
    {
    ...
    }
    {
      ...
      assert(result(1).toSet ==
        Set(
          GroupBy(Some(Nominal("4711", 0)), Aggregated(AggregationOp.Sum, 2, Some(7.0), 1, 4), 4, -1)
        )
      )
    }
    {
    ...
    }
    {
      ...
      assert(
        result.head.toSet ==
          Set(
            GroupBy(Some(Nominal("4711", 0)), Aggregated(AggregationOp.Min, 2, Some(1.0), 1, 3), 3, -1),
            GroupBy(Some(Nominal("4712", 0)), Aggregated(AggregationOp.Min, 2, Some(2.0), 1, 3), 3, -1),
          )
      )
      assert(
        result(1) ==
          List(
            GroupBy(Some(Nominal("4712", 0)), Aggregated(AggregationOp.Sum, 2, Some(2.0), 1, 4), 4, -1)
          )
      )
    }
    {
      ...
      assert(
        result(1) ==
        List(GroupBy(Some(Nominal("4712", 0)), Aggregated(AggregationOp.Sum, 2, Some(2.0), 1, 4), 4, -1))
      )
    }
