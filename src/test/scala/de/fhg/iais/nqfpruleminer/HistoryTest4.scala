...
class HistoryTest4 extends FunSuite {
  ...
  implicit private val ctx: Context = new Context("src/test/resources/historytest4.conf")
  ...
  test("History test") {
    {
      ...
      assert(
        result.head ==
          List(GroupBy(Some(nominal4711), Aggregated(AggregationOp.Min, 2, None, 1, 3), 1, 3))
      )
      assert(
        result(1) ==
          List(
            GroupBy(Some(nominal4711), Counted(Group(List(numeric1, nominalA), -1), Some(1), 1, 4), 1, 4)
          )
      )
    }
    {
    ...
    }
    {
      ...
      assert(result(1).toSet ==
        Set(
          GroupBy(Some(nominal4711), Counted(Group(List(numeric1, nominalA), -1), Some(1), 1, 4), 1, 4),
          GroupBy(Some(nominal4711), Counted(Group(List(numeric3, nominalA), -1), Some(1), 1, 4), 1, 4)
      )
      )
    }
    {
    ...
    }
    {
      ...
      assert(
        result.head.toSet ==
          Set(
            GroupBy(Some(nominal4711), Aggregated(AggregationOp.Min, 2, None, 1, 3), 1, 3),
            GroupBy(Some(nominal4721), Aggregated(AggregationOp.Min, 2, None, 1, 3), 1, 3)
          )
      )
      assert(result(1).toSet ==
        Set(
          GroupBy(Some(nominal4711), Counted(Group(List(numeric1, nominalA), -1), Some(1), 1, 4), 1, 4),
          GroupBy(Some(nominal4711), Counted(Group(List(numeric3, nominalA), -1), Some(1), 1, 4), 1, 4),
          GroupBy(Some(nominal4721), Counted(Group(List(numeric1, nominalA), -1), Some(1), 1, 4), 1, 4)
        )
      )
    }
    {
      ...
      assert(result(1).toSet ==
        Set(
          ...
          GroupBy(Some(nominal4711), Counted(Group(List(numeric3, nominalA), -1), Some(1), 1, 4), 1, 4),
          GroupBy(Some(nominal4721), Counted(Group(List(numeric1, nominalA), -1), Some(1), 1, 4), 1, 4),
          GroupBy(Some(nominal4722), Counted(Group(List(numeric0,nominalA), -1), Some(1), 1, 4), 1, 4)
        )
      )
    }
    {
      ...
      assert(result(1).toSet ==
        Set(
          GroupBy(Some(nominal4711), Counted(Group(List(numeric3, nominalA), -1), Some(1), 1, 4), 1, 4),
          GroupBy(Some(nominal4711), Counted(Group(List(numeric1, nominalB), -1), Some(1), 1, 4), 1, 4),
          GroupBy(Some(nominal4721), Counted(Group(List(numeric1, nominalA), -1), Some(1), 1, 4), 1, 4),
          GroupBy(Some(nominal4722), Counted(Group(List(numeric0,nominalA), -1), Some(1), 1, 4), 1, 4)
        )
      )
    ...
    }
    {
      ...
      assert(result(1).toSet ==
        Set(
          GroupBy(Some(nominal4711), Counted(Group(List(numeric1, nominalB), -1), Some(1), 1, 4), 1, 4),
          GroupBy(Some(nominal4711), Counted(Group(List(numeric2, nominalB), -1), Some(1), 1, 4), 1, 4)
        )
      )
    }
    {
    ...
    }
    {
      ...
      assert(
        result.head.toSet ==
          Set(
            GroupBy(Some(nominal4711), Aggregated(AggregationOp.Min, 2, None, 1, 3), 1, 3),
            GroupBy(Some(nominal4712), Aggregated(AggregationOp.Min, 2, None, 1, 3), 1, 3)
          )
      )
      assert(
        result(1).toSet ==
          Set(
            GroupBy(Some(nominal4711), Counted(Group(List(numeric1, nominalA), -1), Some(1), 1, 4), 1, 4),
            GroupBy(Some(nominal4712), Counted(Group(List(numeric2, nominalA), -1), Some(1), 1, 4), 1, 4)
          )
      )
    }
    {
