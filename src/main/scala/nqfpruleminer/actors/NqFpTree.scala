...
import nqfpruleminer.{Context, Distribution, FPnode, Quality}
...
object NqFpTree {
  ...
  def props(range: Range, numberOfItems: Int, lengthOfSubgroups: Int, master: ActorRef, bestSubgroups: ActorRef)(implicit ctx: Context): Props =
...
class NqFpTree(lower: Int, upper: Int, numberOfItems: Int, lengthOfSubgroups: Int, master: ActorRef, bestSubgroups: ActorRef)(implicit ctx: Context) extends Actor with ActorLogging {
  ...
  private val root = FPnode(-1, None)
