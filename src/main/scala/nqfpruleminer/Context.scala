      ...
      case Success(conf) =>
        ...
        val dateTimeFormatter =
          Try(conf.getString("format")) match {
            ...
            case Failure(e) => fail(s"Time format is missing: ${e.getMessage}"); null
          }
          ...
          val attributes =
            if (operator == AggregationOperator.count || operator == AggregationOperator.exists) {
              Try(config.getStringList("attributes").asScala.toList) match {
                ...
                case Failure(_) => fail(s"For operator 'count' or 'exists' the key 'attributes' is required."); null
              }
            } else {
            ...
            .map {
              period =>
                if (operator == AggregationOperator.exists || operator == AggregationOperator.count) {
                  val name = s"Aggregate(${period.toString})"
                  val minimum: Int =
