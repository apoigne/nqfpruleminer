...
object DerivedType {
  ...
  case class AGGREGATE(seqIdPos: Option[Int], position: Int, op: AggregationOp.Value, existsOnly: Boolean,
                       condition: Numeric => Boolean, binning: BinningType, history: Either[Int, TimeFrame]
...
case class Counted(item: Item, result: Option[Int], label: Label, position: Position = -1)
  ...
  override def toString: String =
    result match {
      ...
      case Some(v) =>
        s"${ctx.allFeatures(position).name}.count(${item.toString}) == $v" +
          s"${if (ctx.numberOfTargetGroups <= 2) "" else s" & label == ${ctx.targetGroups(label - 1)}"}"
    }
}
