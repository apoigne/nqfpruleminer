...
trait History {
  ...
  def update(actualTime: DateTime, baseItems: Vector[Valued], label: Value.Label): Unit
...
trait HistoryByTimeframe extends History {
  ...
  private val period: Int = aggr.history match {case Left(_) => 0; case Right(t) => t}

  def apply(label: Value.Label, actualTime: DateTime, baseItems: Vector[Valued]): Vector[Item] = {
    if (aggr.condition(baseItems.map(_.value))) update(actualTime, baseItems, label)
    if (actualTime.compareTo(lastTime) > 0) {
      history = history.filterNot(elem => actualTime.minusSeconds(period).compareTo(elem.dateTime) > 0)
      lastTime = actualTime
    }
    getAggregatedValues(history)
  }
}
