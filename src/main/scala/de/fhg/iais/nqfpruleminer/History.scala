...
trait History {
  ...
  val lengthOfHistory: Int = aggr.history.offset + aggr.history.length
...
trait HistoryByTimeframe extends History {
  ...
  def apply(label: Value.Label, actualTimeOption: Option[DateTime], baseItems: Vector[Valued]): Vector[Item] = {
    ...
    getAggregatedValues(history.filter(elem => {actualTime.minusSeconds(aggr.history.offset).compareTo(elem.dateTime.get) > 0}))
  }
}
...
trait HistoryByLength extends History {
  ...
  def apply(label: Value.Label, actualTime: Option[DateTime], baseItems: Vector[Valued]): Vector[Item] = {
    val aggregatedValues = getAggregatedValues(history.zip(conditions).drop(aggr.history.offset).filter(_._2).map(_._1))
