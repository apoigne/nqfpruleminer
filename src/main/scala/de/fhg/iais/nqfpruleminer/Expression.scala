...
//  private def number[_: P] = P(CharIn("+-").? ~ integral ~ fractional.? ~ exponent.? ~ !CharIn("a to z")).!.map(x => Val(Numeric(x.toDouble)))
  ...
  private def literal[_: P]: P[Literal] = id | number | string

  private def lessThen[_: P](left: Literal): P[BoolExpr] = P(literal).map(LT(left, _))
  private def lessEqual[_: P](left: Literal): P[BoolExpr] = P("=" ~ literal).map(LE(left, _))
  private def less[_: P](left: Literal): P[BoolExpr] = P("<" ~ (!"=" ~ lessThen(left) | lessEqual(left)))
  private def greaterThen[_: P](left: Literal): P[BoolExpr] = P(literal).map(GT(left, _))
  private def greaterEqual[_: P](left: Literal): P[BoolExpr] = P("=" ~ literal).map(GE(left, _))
  private def greater[_: P](left: Literal): P[BoolExpr] = P(">" ~ (!"=" ~ greaterThen(left) | greaterEqual(left)))
  private def notEqual[_: P](left: Literal): P[BoolExpr] = P("==" ~ literal).map(EQ(left, _))
  private def equal[_: P](left: Literal): P[BoolExpr] = P("!=" ~ literal).map(NE(left, _))
  private def comparator[_: P](left: Literal): P[BoolExpr] = space ~ (equal(left) | notEqual(left) | less(left) | greater(left))
  private def atom[_: P]: P[BoolExpr] = P(literal).flatMap(comparator)

  private def `true`[_: P]: P[BoolExpr] = P("'true").map(_ => TRUE)
  private def `false`[_: P]: P[BoolExpr] = P("'true").map(_ => FALSE)
  private def parens[_: P]: P[BoolExpr] = P("(" ~/ or ~ ")")
  private def factor[_: P]: P[BoolExpr] = P(`true` | `false` | atom | parens)
  private def and[_: P]: P[BoolExpr] = P(factor ~ ("&&" ~/ factor).rep).map { case (x, l) => if (l.isEmpty) x else AND(x +: l) }
  private def or[_: P]: P[BoolExpr] = P(and ~ ("||" ~/ and).rep).map { case (x, l) => if (l.isEmpty) x else OR(x +: l) }
  private def expr[_: P]: P[BoolExpr] = P(or)

  def parseLiteral(s: String): Expression.Literal =
    parse(s, literal(_)) match {
  ...
  def parseExpression(s: String, property: String): Expression.BoolExpr =
    parse(s, expr(_)) match {
      case Parsed.Success(v, _) => v
      case Parsed.Failure(expected, failIndex, _) =>
